      ...
      <div className='flex flex-col md:flex-row'>
        <div className='md:min-h-[60vh] lg:w-2/4 md:w-3/4 w-full bg-[#16520f] flex pt-8 relative justify-center'>
          <div className='flex flex-col gap-8 p-4  items-center lg:left-[7%] md:left-[8%] md:top-40 md:sticky md:rounded-l-2xl shadow-[0_0_40px_#000000] lg:h-[60%] h-[50%] w-[80%] bg-white text-white relative'>
            ...
            <span onClick={() => { WishlistBtnClick(wishlistInfo?._id) }} className='cursor-pointer absolute top-6 right-6 text-[20px] bg-white rounded-full h-8 w-8 justify-center items-center flex'>{wishlistInfo?.isWishlist ? "❤️" : "🤍"}</span>
            <div className='flex w-full justify-around'>
              <div onClick={() => { cartBtnClick() }} className='flex gap-2 items-center bg-green-500 text-[10px] lg:text-[14px]  lg:px-5 py-1 px-2 rounded-md cursor-pointer'>
                <img src="/addToCart.gif" alt="cart" className='lg:w-8 w-6 invert' />
                <span>ADD TO CART</span>
              </div>
              <div onClick={()=>{buyBtnClick()}} className='flex gap-2 items-center bg-green-500 text-[10px] lg:text-[14px] lg:px-5 py-1 px-2 rounded-md cursor-pointer'>
